...
func RegisterRoutes(guarded *mux.Router, database *db.MongoDB, auth *middleware.AuthMiddleware, tenant *middleware.TenantMiddleware, logger *syslog.Logger, senders ...email.AttachmentSender) {
	...
	productAPI.Handle("/locations/{locationId}/forecast/shadow-kpis", withProductMiddleware(http.HandlerFunc(handler.listShadowKPIReports), requireProfile, forecastLocation, RequireBusinessPermission(models.PermissionForecastRead))).Methods(http.MethodGet)
	productAPI.Handle("/locations/{locationId}/forecast/shadow-kpis", withProductMiddleware(http.HandlerFunc(handler.saveShadowKPIReport), requireProfile, forecastLocation, RequireBusinessPermission(models.PermissionForecastManage), requireForecastManager())).Methods(http.MethodPost)
	productAPI.Handle("/locations/{locationId}/forecast/shadow-kpis/{reportId}", withProductMiddleware(http.HandlerFunc(handler.listShadowKPIReports), requireProfile, forecastLocation, RequireBusinessPermission(models.PermissionForecastRead))).Methods(http.MethodGet)
	// Keep the report-oriented spelling available for operational clients while
	// retaining the concise shadow-kpis path used by the forecast API.
	productAPI.Handle("/locations/{locationId}/forecast/shadow-kpi-reports", withProductMiddleware(http.HandlerFunc(handler.listShadowKPIReports), requireProfile, forecastLocation, RequireBusinessPermission(models.PermissionForecastRead))).Methods(http.MethodGet)
	productAPI.Handle("/locations/{locationId}/forecast/shadow-kpi-reports", withProductMiddleware(http.HandlerFunc(handler.saveShadowKPIReport), requireProfile, forecastLocation, RequireBusinessPermission(models.PermissionForecastManage), requireForecastManager())).Methods(http.MethodPost)
	productAPI.Handle("/locations/{locationId}/forecast/shadow-kpi-reports/{reportId}", withProductMiddleware(http.HandlerFunc(handler.listShadowKPIReports), requireProfile, forecastLocation, RequireBusinessPermission(models.PermissionForecastRead))).Methods(http.MethodGet)
