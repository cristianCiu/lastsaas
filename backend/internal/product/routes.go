...
func RegisterRoutes(guarded *mux.Router, database *db.MongoDB, auth *middleware.AuthMiddleware, tenant *middleware.TenantMiddleware, logger *syslog.Logger) {
	...
	productAPI.HandleFunc("/branding", handler.getTenantBranding).Methods(http.MethodGet)
	productAPI.HandleFunc("/branding/assets", handler.listTenantBrandingAssets).Methods(http.MethodGet)
	productAPI.HandleFunc("/branding/assets/{kind}", handler.serveTenantBrandingAsset).Methods(http.MethodGet)
	...
	productAPI.Handle("/branding", withProductMiddleware(http.HandlerFunc(handler.putTenantBranding), middleware.RequireRole(models.RoleAdmin))).Methods(http.MethodPut)
	productAPI.Handle("/branding/assets/{kind}", withProductMiddleware(http.HandlerFunc(handler.putTenantBrandingAsset), middleware.RequireRole(models.RoleAdmin))).Methods(http.MethodPut)
	productAPI.Handle("/branding/assets/{kind}", withProductMiddleware(http.HandlerFunc(handler.deleteTenantBrandingAsset), middleware.RequireRole(models.RoleAdmin))).Methods(http.MethodDelete)
