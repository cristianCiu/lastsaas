		...
		},
		{
			...
			Endpoints: []apiEndpoint{
				...
				},
				{
					...
					Detail:   "Creates an active location. The code must be lower-case and unique within the tenant, the timezone must be an IANA timezone, and the tenant's max_locations entitlement is enforced. Requires owner or admin role.",
					Auth:     "admin",
					Body:     `{"code":"berlin","name":"Berlin","timezone":"Europe/Berlin"}`,
					Response: `{"location":{"id":"...","code":"berlin","name":"Berlin","timezone":"Europe/Berlin","isActive":true,"version":1,"createdAt":"...","updatedAt":"..."}}`,
				},
				{
					Method:   "PATCH",
					Path:     "/api/product/locations/{locationId}",
					Summary:  "Update or deactivate a tenant location",
					Detail:   "Updates a location using optimistic concurrency. Version is required and must match the current location version; code is immutable. At least one of name, timezone, or isActive is required. Requires owner or admin role. A stale version returns VERSION_CONFLICT; locations outside the current tenant return NOT_FOUND.",
					Auth:     "admin",
					Params:   []apiParam{{"locationId", "ObjectID", true, "The tenant location ID"}},
					Body:     `{"version":1,"name":"Berlin Mitte","timezone":"Europe/Berlin","isActive":true}`,
					Response: `{"location":{"id":"...","code":"berlin","name":"Berlin Mitte","timezone":"Europe/Berlin","isActive":true,"version":2,"createdAt":"...","updatedAt":"..."}}`,
				},
			},
		},
		{
		...
		},
		{
			...
			Endpoints: []apiEndpoint{
				...
				},
				{
					...
					Detail:   "Creates a new webhook with an auto-generated signing secret (prefixed <code>whsec_</code>). The full secret is returned only in this response; regenerating it invalidates the current secret. All deliveries include an <code>X-Webhook-Signature</code> header containing the HMAC-SHA256 signature of the payload.",
					Auth:     "admin",
				...
				},
				{
					...
					Detail:   "Returns webhook configuration with a secret preview and the 20 most recent delivery attempts. The full signing secret is never returned after creation or regeneration.",
					Auth:     "admin",
					Params:   []apiParam{{"webhookId", "ObjectID", true, "Webhook ID"}},
					Response: `{"webhook":{"id":"...","name":"Provisioning","secretPreview":"abcd1234",...},"deliveries":[{"id":"...","eventType":"tenant.created","payload":"{...}","responseCode":200,"responseBody":"ok","success":true,"durationMs":120,"createdAt":"..."}]}`,
				},
				{
					Method:   "PUT",
					Path:     "/api/admin/webhooks/{webhookId}",
