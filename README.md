...
The current application version is **7.0.0**. `VERSION` is the runtime version source, while this ledger ties product evolution to immutable Git commits. Planned versions are targets and retain `TBD` until implementation is committed.
...
| 7.0.0 | [`240fed2`](https://github.com/cristianCiu/lastsaas/commit/240fed2) | Pilot-hardening preparation: privacy offboarding, worker deployment configuration, worker health, shadow KPIs, and accessibility baseline. External pilot gates remain incomplete. |
